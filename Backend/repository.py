...
def get_video_project(session: Session, project_id: str) -> Optional[VideoProject]:
    return session.get(VideoProject, project_id)


def list_video_projects(session: Session, limit: int = 50) -> list[VideoProject]:
    stmt = select(VideoProject).order_by(VideoProject.created_at.desc()).limit(limit)
    return list(session.scalars(stmt).all())


def update_video_project_timeline(
    session: Session,
    project_id: str,
    timeline_json: dict,
    status: str = "planned",
...
def update_video_project_status(
    ...
    project.status = status
    if metadata_patch:
        current_metadata = dict(project.metadata_json or {})
        current_metadata.update(metadata_patch)
        project.metadata_json = current_metadata
    project.updated_at = utcnow()
    session.commit()
    session.refresh(project)
    return project
